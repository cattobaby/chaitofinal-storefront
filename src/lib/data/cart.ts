...
export async function updateRegionWithValidation(
  ...
  if (cartId) {
    ...
    try {
      ...
      // ✅ AUTO-HEALING: Si el carrito no se encuentra en DB, limpiamos la cookie y seguimos.
      if (error?.message?.includes("not found") || error?.message?.includes("Cart with id") || error?.status === 404) {
        console.log(`${LOG} Cart not found in DB, removing ghost cookie...`)
        await removeCartId()
        return {
          removedItems: [],
