    ...
    try {
      ...
      const { response } = await listProducts({
        ...
        queryParams: {
          ...
          handle: items.map((item: any) => item.handle),
          limit: items.length,
        },
      })
  ...
  return (
    <div className="min-h-[70vh]">
      <div className="flex justify-between w-full items-center">
        <div className="my-4 label-md">{`${count} resultados`}</div>
      </div>
          ...
          {!items.length ? (
            <div className="text-center w-full my-10">
              ...
              <p className="mt-4 text-lg">
                Lo sentimos, no podemos encontrar resultados para tus criterios
              </p>
            </div>
          ) : (
            <div className="w-full">
                  ...
                  (hit: any) =>
                    apiProducts?.find((p: any) => p.id === hit.objectID) && (
                      <ProductCard
                        ...
                        api_product={apiProducts?.find((p: any) => p.id === hit.objectID)}
                        key={hit.objectID}
