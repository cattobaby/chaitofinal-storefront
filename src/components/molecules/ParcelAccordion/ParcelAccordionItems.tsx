...
function labelForFulfillmentStatus(s?: string | null) {
    switch (s) {
        case "not_fulfilled":
            return "Recibido"
        case "fulfilled":
...
export const ParcelAccordionItems = ({
    ...
    const statusLabel = labelForFulfillmentStatus(order?.fulfillment_status)
            ...
            {convertToLocale({ amount: order.shipping_total, currency_code })}
                ...
                <div className="flex items-center gap-4 justify-end">
                    <p className="label-md">
                        {totalItems === 1 ? `${totalItems} artículo` : `${totalItems} artículos`}
                    </p>
                    <CollapseIcon
                        size={20}
                        className={cn(
